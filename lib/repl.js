 ...
 */
function KnowSheetBricksREPLServer(options) {
	// Credits to the `REPLServer` source code for some of the following snippets.
	
	// Support instantiation without `new`.
	if (!(this instanceof KnowSheetBricksREPLServer)) {
		return new KnowSheetBricksREPLServer(options);
	}
	...
	this.eval = function (code, context, file, cb) {
		defaultEval(code, context, file, function (err, evalResult) {
			...
			// Async evaluation.
			when(evalResult).then(function (actualResult) {
...
inherits(KnowSheetBricksREPLServer, REPLServer);

KnowSheetBricksREPLServer.prototype.createContext = function () {
