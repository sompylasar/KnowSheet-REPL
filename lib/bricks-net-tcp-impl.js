...
function Connection(socket) {
	...
	var _readPromiseChain = when.resolve();
	var _writePromiseChain = when.resolve();
	...
	_this.BlockingRead = function (buffer, offset, maxBytesToRead, fillFullBuffer) {
		...
		//argsString = [].map.call(arguments, function (arg) {
		//	return (arg instanceof Buffer ? 'Buffer(' + arg.length + ')' : inspect(arg));
		//}).join(', ');
		
		_this._DEBUG_LOG(_this + '#BlockingRead(' + argsString + ').');
		
		_readPromiseChain = _readPromiseChain.then(function () {
			return when.promise(function (resolve, reject) {
				_this._DEBUG_LOG(_this + '#BlockingRead(' + argsString + '): Read.');
		...
		return _readPromiseChain;
	};
	
	_this.BlockingWrite = function (chunk) {
		_this._DEBUG_LOG(_this + '#BlockingWrite(' + inspect(chunk) + ').');
		
		_writePromiseChain = _writePromiseChain.then(function () {
			return when.promise(function (resolve, reject) {
				_this._DEBUG_LOG(_this + '#BlockingWrite(' + inspect(chunk) + '): Write.');
			
				_socket.write(chunk, 'utf8', function (err) {
					_this._DEBUG_LOG(_this + '#BlockingWrite(' + inspect(chunk) + '): Write callback:', err || 'OK');
