		...
		function consume() {
			if (readRequest) {
				...
				if (internalBuffersLength > 0) {
					if (fillFullBuffer && internalBuffersLength < maxBytesToRead) {
		...
		function part5(next_crlf_ptr) {
			...
			if (!first_line_parsed) {
				...
				if (!line_is_blank) {
					...
					if (chunk_length === 0) {
						_this._DEBUG_LOG('TemplatedHTTPRequestData#then part5 OnChunkedBodyDone');
						...
						if (offset < next_offset) {
							...
							when(read(bytes_to_read, true)).done(function (read_count) {
								...
								_this._DEBUG_LOG('TemplatedHTTPRequestData#then part5 after read OnChunk');
						...
						_this._DEBUG_LOG('TemplatedHTTPRequestData#then part5 offset >= next_offset OnChunk');
						_this.OnChunk(_this.buffer_.toString('utf8', chunk_offset, chunk_offset + chunk_length));
						next_line_offset = next_offset;
					}
				}
				...
				if (p >= 0) {
					...
					_this._DEBUG_LOG('TemplatedHTTPRequestData#then part5 else !line_is_blank p >= 0 OnHeader',
...
TemplatedHTTPRequestData.prototype.OnHeader = function (key, value) {
	throw new Error('TemplatedHTTPRequestData#OnHeader: MUST BE OVERRIDDEN.');
};
TemplatedHTTPRequestData.prototype.OnChunk = function (chunk) {
	throw new Error('TemplatedHTTPRequestData#OnChunk: MUST BE OVERRIDDEN.');
};
TemplatedHTTPRequestData.prototype.OnChunkedBodyDone = function () {
	throw new Error('TemplatedHTTPRequestData#OnChunkedBodyDone: MUST BE OVERRIDDEN.');
};
