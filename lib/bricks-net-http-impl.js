...
function TemplatedHTTPRequestData(connection) {
	...
	(function () {
		...
		function strstr(buffer, str, start) {
			...
			//	require('util').inspect(buffer.toString('utf8')),
			//	require('util').inspect(str),
			//	start
			//);
		...
		function finish(err) {
			_this._DEBUG_LOG('TemplatedHTTPRequestData finish:', (err || 'OK'));
		...
		function part5(next_crlf_ptr) {
			...
			if (!first_line_parsed) {
				...
				if (!line_is_blank) {
					...
					_this._DEBUG_LOG('TemplatedHTTPRequestData: part5',
						'line ===', require('util').inspect(line),
						'pieces ===', pieces);
...
function ChunkedResponseSender(connection) {
	if (!(this instanceof ChunkedResponseSender)) {
		return new ChunkedResponseSender(connection);
	}
	...
	// TODO(sompylasar): Implement ChunkedResponseSender.
...
function HTTPServerConnection(connection) {
	...
	Object.defineProperties(_this, {
		...
		toString: {
			...
			enumerable: false,
			value: function () {
				return 'HTTPServerConnection(' + this.connection_ + ')';
			}
		},
		destroy: {
			configurable: true,
...
HTTPServerConnection.prototype.SendChunkedHTTPResponse = function (
	...
	if (_this.responded_) {
		...
		headers += "Transfer-Encoding: chunked" + kCRLF;
		
		return when(_this.connection_.BlockingWrite(headers)).then(function () {
			return ChunkedResponseSender(_this.connection_);
		});
	}
};
